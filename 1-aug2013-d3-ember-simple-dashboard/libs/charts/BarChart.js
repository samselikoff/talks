    ...
    function chart(selection) {
        selection.each(function(data)
        {
            ...
            if (hoverDataLabels && !hideDataLabels ) {
                ...
                selection
                    .selectAll(".bar")
                    .on("mouseover", function() { hoverLabel.style("display", "initial"); })
                    .on("mouseout", function() { hoverLabel.style("display", "none"); })
                    .on("mousemove", function(d, i) {
                        ...
                        var $hoverLabel = hoverLabel[0][0],
                            ...
                            top = pos.top + window.scrollY - $hoverLabel.offsetHeight - 6,
                            left = pos.left + window.scrollX - ($hoverLabel.offsetWidth / 2) + (pos.width / 2);
            ...
            if (fadeOnHover) {
                selection.selectAll(".bar")
                    .on("mouseout.fade", function() {
                        selection.selectAll(".bar").transition().duration(100).ease("linear").style("opacity", "1");
                    })
                    .on("mouseover.fade", function(d, i) {
                        selection.selectAll(".bar").filter(function(d, j){return i!=j;})
                            .transition().duration(100).ease("linear").style("opacity", ".5");
                        selection.selectAll(".bar").filter(function(d, j){return i==j;})
                            .transition().duration(100).ease("linear").style("opacity", "1");
                    });
            }
        });
    }
